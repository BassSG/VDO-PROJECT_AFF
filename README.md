...
## Workflow v1.7.0 · Protected Delivery + Cost Guard
...
5. ก่อนเริ่ม ระบบแสดงราคาประมาณของ **AI ทุกขั้นตอนรวมกัน** และขอให้ผู้ใช้ยืนยัน จากนั้นเรียกเฉพาะ Creative Plan และ Key Visual ก่อน
6. ระบบหยุดที่ Cost Guard ให้ตรวจ Key Visual, บทพูด และค่าวิดีโอแยกอีกครั้ง โดยยังไม่ส่ง Veo และยังไม่คิดค่าวิดีโอ
7. ผู้ใช้ต้องยืนยันว่าบุคคลในภาพเป็นผู้ใหญ่อายุ 18 ปีขึ้นไปหรือไม่มีเด็ก/เยาวชน แล้วกดยืนยันค่าวิดีโอ ระบบจึงส่งงานวิดีโอเพียงครั้งเดียว ปุ่มถูกล็อกและสถานะฝั่งเซิร์ฟเวอร์ป้องกันการแตะซ้ำ
8. OpenRouter ใช้ Veo สร้าง MP4 พร้อมเสียงพูดในวิดีโอเดียว เพื่อลดปัญหาเสียงแยกไม่ตรงปาก
9. Auto Review ใช้ Whisper ฟังเสียงภาษาไทยแล้วเทียบกับ **บทที่อนุมัติจริง** หากผลยังไม่ชัดจะ Auto Recheck ด้วยโมเดลตัวที่สองอีกหนึ่งครั้ง
10. ระบบตรวจโครงสร้าง MP4 และบังคับ codec ที่ Browser มือถือรองรับ (`H.264/AVC + AAC`) ก่อนเปิดให้ใช้งาน พร้อมแสดงค่าใช้จ่ายจริงแยกตามขั้นตอนเมื่อ provider ส่ง usage กลับมา
11. ปุ่มดูและดาวน์โหลดประกอบไฟล์เป็น Browser Blob โดยตรง ไม่ต้องรอ Google Drive แปลง Preview; ลิงก์ Drive เป็นทางสำรองเท่านั้น
12. ถ้า Auto Review หรือขั้นหลังสร้างมีปัญหา ไฟล์ที่สร้างและบันทึกแล้วจะถูกกู้ขึ้นมาเป็น `NEEDS REVIEW` และยังเปิด/ดาวน์โหลดได้เสมอ ระบบจะ **ไม่สร้างวิดีโอใหม่และไม่หักเงินรอบใหม่เอง**

เคสงานเดิมที่ถูก Auto Review ปฏิเสธเพราะตรวจชื่อสินค้าเกินกว่าบทที่อนุมัติ จะถูกคำนวณผลใหม่จาก transcript ที่บันทึกไว้โดยไม่เรียก AI เพิ่มและไม่เพิ่มค่าใช้จ่าย เช่น transcript 94% ที่ต่างกันเพียง `สิบ` กับ `10` จะผ่านได้เมื่อบทไม่ได้กำหนดให้พูดชื่อแคมเปญเต็ม
...
การยืนยันรอบแรกอาจมีค่า Creative Plan และ Key Visual ส่วนค่าวิดีโอจะเริ่มได้ต่อเมื่อผู้ใช้ตรวจภาพและกดยืนยันใน Cost Guard เท่านั้น หากการส่งงานไม่คืนรหัสงาน ระบบจะหยุดและไม่ retry อัตโนมัติ ผู้ใช้ควรตรวจ OpenRouter Activity/Usage ก่อนลองใหม่ เพราะกรณีเครือข่ายขาดช่วงไม่สามารถสรุปจากหน้าแอปอย่างเดียวได้ว่า provider ไม่มีค่าใช้จ่าย
...
npx --yes @google/clasp@latest -u project-owner deploy \
  ...
  --description "ADORA v1.7.0 - Protected Delivery and paid-video Cost Guard"
```
